## **Тестовое задание QA Avito**

---

**Задание 1**

[Баги, наденные в задании](https://github.com/l1fecoaching/QA-trainee-assignment-autumn-2025/blob/main/TASK1.md)

---

**Задание 2**

[Тест-кейсы](https://github.com/l1fecoaching/QA-trainee-assignment-autumn-2025/blob/main/TESTCASES.md)

[Баги](https://github.com/l1fecoaching/QA-trainee-assignment-autumn-2025/blob/main/BUGS.md)

Все автотесты пройдены. Есть баги, поэтому некоторые автотесты не проходят. Если их отключить, все тесты будут пройдены.

---
#### Требования

Установить [python](https://www.python.org/downloads/) по инструкции



#### **Установка зависимостей**

```
pip install pytest
```
```
pip install requests
```
**Запустить все автотесты одной командой**
```
pytest tests/test_api.py
```
**или конкретный автотест командой**
```
pytest tests/test_api.py::название теста
```
